...
let scena, heroj, ork, zmaj, kutija, pozadina, herojStatus, orkStatus, zmajStatus
...
class Karakter extends Sprite {
  ...
  pokaziStatistike() {
    return `
      <strong><span id='ime'>${this.ime}</span></strong><br />
      preciznost: ${this.preciznost}<br />
      udarac: ${this.udarac}<br />
      eskivaža: ${this.eskivaza}<br />
      oklop: ${this.oklop}<br />
      zdravlje: <span id='zdravlje'>${this.zdravlje}</span>
    `
  }

  borba(protivnik) {
    const preciznost = (this.preciznost - protivnik.eskivaza) / 100
    if (Math.random() > preciznost) return
    let udarac = 0
    for (let i = 0; i < this.udarac; i++) {
      udarac += parseInt(Math.random() * 6)
    }
    udarac -= protivnik.oklop
    protivnik.zdravlje -= udarac
    if (protivnik.zdravlje > 0) return
